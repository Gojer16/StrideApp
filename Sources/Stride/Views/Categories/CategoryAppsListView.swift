...
/**
 * CategoryAppsListView - Displays apps assigned to a specific category
 *
 * Aesthetic: Warm Paper/Editorial Light
 * - Clean white cards with soft shadows
 * - Terracotta accents matching category colors
 * - Animated app list with staggered reveals
...
struct CategoryAppsListView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showingAssignApps, onDismiss: {
            // Refresh the apps list when the sheet closes
            self.loadCategoryApps()
            // Notify parent that apps may have changed
            self.onAppsChanged?()
        }) {
            AssignAppsToCategoryView(category: category, onComplete: {
                // Also refresh when apps are assigned
                self.loadCategoryApps()
                // Notify parent that apps have changed
                self.onAppsChanged?()
            })
        }
    }
    ...
    private var headerView: some View {
        ...
        .background(
            LinearGradient(
                colors: [
                    Color(hex: category.color).opacity(0.06),
                    Color.clear
                ],
                startPoint: .top,
                endPoint: .bottom
        ...
        VStack(spacing: 20) {
            ...
            ZStack {
                Circle()
                    .fill(
                        RadialGradient(
                            colors: [Color(hex: category.color).opacity(0.12), Color.clear],
                            center: .center,
                            startRadius: 0,
                            endRadius: 60
                        )
                ...
                Image(systemName: "app.badge.checkmark")
                    ...
                    .foregroundColor(Color(hex: category.color).opacity(0.7))
            }
            ...
            VStack(spacing: 8) {
                ...
                Text("Add apps to track their time in this category")
                    .font(.system(size: 13))
                    .foregroundColor(secondaryText)
            }
                ...
                Text("Assign Apps")
                    ...
                    .background(
                        Capsule()
                            ...
                            .overlay(
                                Capsule()
                                    .strokeBorder(Color(hex: category.color).opacity(0.3), lineWidth: 1)
                            )
                    )
                    .foregroundColor(Color(hex: category.color))
            }
            ...
            Section {
                ForEach(Array(apps.enumerated()), id: \.element.id) { index, app in
                    ...
                    .onHover { isHovered in
                        hoveredAppId = isHovered ? app.id : nil
                    }
...
private struct AppRowItem: View {
    ...
    var body: some View {
        HStack(spacing: 14) {
            ...
            .background(
                Capsule()
                    .fill(isHovered ? Color(hex: categoryColor).opacity(0.15) : Color.black.opacity(0.04))
            )
